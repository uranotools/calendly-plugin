...
// Plugins/Availability/AvailabilityPlugin.ts
var AvailabilityPlugin_exports = {};
__export(AvailabilityPlugin_exports, {
  AvailabilityPlugin: () => AvailabilityPlugin
});
module.exports = __toCommonJS(AvailabilityPlugin_exports);
  ...
  async executeAction(action, payload) {
    ...
    if (action === "check") {
      ...
      return {
        ...
        available_times: slots.map((slot) => ({
          start_time: slot.start_time,
          invitees_remaining: slot.invitees_remaining,
          status: slot.status
        })),
        summary: slots.length > 0 ? `Hay ${slots.length} horario(s) disponible(s) entre ${start_time} y ${end_time}.` : `No hay horarios disponibles entre ${start_time} y ${end_time}.`
      };
    }
    throw new Error(`Acci\xF3n '${action}' no soportada en AvailabilityPlugin`);
  }
};
